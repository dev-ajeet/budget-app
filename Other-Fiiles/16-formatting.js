/********************
 * Formatting our Budget Number: string manipulation
 * How to use different Strings methods to manipulate strings. 
 * First Rule: All numbers have a decimal part  with exactly two numbers, even if they are originally integers. 
 * Second Rule: All the expanses always have the minus sign in front of them, and then the space. While all the income number are positive number so they have + sign, and also a space.
 * If a number is in thousands, then we should have the commas at proper place. 
 * 
 * So we are going to add a method to the UI controller, and then each time we display a number on the user interface, we are going to call that method and input the number into that method, and then output the formatted number. 
 *
 */
...
var budgetController = (function() {
  ...
  return {
    ...
    getPercentages: function() {
      //this is how we export particular value from an array of objects.
...
var uiController = (function() {
  ...
  var formatNumber = function(num, type){
           ...
           * 2000 -> 2,000.00
           */
          num = Math.abs(num); //Overriding the num. Just take out the absolute part. i.e Number without the sign.
          num = num.toFixed(2);//THis is not a method of math object, but instead it is a method of number prototype, as we know, the strings and number can also have methods, even if they are originally primitive data types. But if we attempt to use methods on them, such as we do here, then JavaScript automatically converts them to objects, and then we can use their methods just like here. 
          //Adding the commas
          numSplit = num.split('.'); //Separates numbers from decimal and it will be stored in an array. 
          int = numSplit[0]; //here int is a string
          dec = numSplit[1];
          if(int.length > 3){
            //     int = int.substr(0, 1); 
            //in this first argument is the index number where we want to start and second number is how many characters we want. So 0, 1 means we are going to start at index 0 and read one element.
  ...
  return {
    ...
    displayPercentages: function(percentages) {
      var fields = document.querySelectorAll(DOMstrings.expensesPercLabel); //from this we will get an array of objects, containing all the elements having the specified class in the whole document.
      console.log("Inside displayPercentages");
      //and this again returns a list, actually it is called a 'node list' because in the DOMtree, where all of the HTML elements of our page are stored, each element is called node. So we need to loop over all of these elements in our selections, i.e. of these nodes, and then change the text content property for all of them.
      //But nodeList does not have the forEach method, so we could again use the hack.
      console.log(fields);
      ...
      }; //when we want same function to be applied on different set of values we can use callback function.
      ...
      /*******
       * So when we call the nodeListForEach function, we pass a callback function into it.
       * This function is the assign to the callback parameter and then we loop over the list, and in each iteration the callback function get called. And we have also access to the current element and current index, because we pass then into the callback.
       */
    },
